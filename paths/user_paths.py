...
def user_home(db, id):
    ...
    helper.db_helper.check_auth(id, AccType.USER, db)
    ...
    user_loans, user_prof_pic) = helper.user.get_user_details(db, id)
...
def pay_fees(db, user_id):
    ...
    paid = helper.user.get_user_paid(db, user_id)
    paid += float(fees_paid)
    helper.user.put_user_paid(db, user_id, paid)
...
def user_account(db, user_id):
    helper.db_helper.check_auth(user_id, AccType.USER, db)
    helper.db_helper.check_fines(db, user_id)
    
    (user_id, user_first_name, user_last_name, user_loan_count,
    user_loans, user_prof_pic) = helper.user.get_user_details(db, user_id)
    user_join_date = helper.user.get_user_join_date(db, user_id)
    user_past_loans = helper.user.get_user_past_loans(db, user_id)
...
def user_book_details(db, user_id, book_id):
    ...
    book_loaned, due_date = helper.loan.get_user_book_details(db, user_id, book_id)
...
def borrow(db, user_id, book_id):
    helper.db_helper.check_auth(user_id, AccType.USER, db)
    helper.loan.create_loan(db, user_id, book_id)
...
def return_book(db, user_id, book_id):
    helper.db_helper.check_auth(user_id, AccType.USER, db)
    helper.loan.end_loan(db, user_id, book_id)
...
def renew_book(db, user_id, book_id):
    helper.db_helper.check_auth(user_id, AccType.USER, db)
    loan_id = helper.book.find_loan_id(db, user_id, book_id)

    helper.loan.renew_loan(db, loan_id)
...
def close_user_account(db, user_id):
    helper.db_helper.check_auth(user_id, AccType.USER, db)
    (user_id, user_first_name, user_last_name, user_loan_count,
    user_current_loans, user_prof_pic) = helper.user.get_user_details(db, user_id)
...
def edit_user_account(db, user_id):
    helper.db_helper.check_auth(user_id, AccType.USER, db)
    message = helper.cookies.get_cookie(EDIT_ACC_COOKIE)
...
def edit_user_account_post(db, user_id):
    ...
    if prof_pic:
        try:
            ...
            helper.cookies.set_cookie(EDIT_ACC_COOKIE,
